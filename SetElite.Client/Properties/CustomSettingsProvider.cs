...
namespace SetElite.Client.Properties
{
	internal class CustomSettingsProvider : SettingsProvider
	{
        ...
        /// </summary>
        public CustomSettingsProvider(string userConfigFilePath = null)
		{
			SettingsDictionary = new Dictionary<string, SettingStruct>();
			UserConfigPath = userConfigFilePath;
		}

		public override string Name => "UserSettingsProvider";
        ...
        ///   e.g. <c>Properties.Configurator.Default.SettingsKey = @"C:\temp\user.config";</c>
        /// </summary>
        private string UserConfigPath { get; }
			...
			foreach (SettingsProperty setting in collection)
			{
				...
				if (SettingsDictionary.ContainsKey(setting.Name))
				{
					...
					value.PropertyValue =
						Convert.ChangeType(
							t.IsEnum
								? Enum.Parse(t, SettingsDictionary[setting.Name].value)
								: SettingsDictionary[setting.Name].value, t);
				}
				else //use defaults in the case where there are no settings yet
				{
					...
					value.PropertyValue =
						Convert.ChangeType(
							t.IsEnum ? Enum.Parse(t, (string) setting.DefaultValue) : setting.DefaultValue, t);
				}
        ...
        /// <summary>
        ///   Must override this, this is the bit that does the saving to file.  Called when Configurator.Save() is called
        /// </summary>
        /// <param name="context"></param>
        /// <param name="collection"></param>
        ...
        private void LoadValuesFromFile()
		{
			...
			var settingElements = configXml.Element(CONFIG)
				?.Element(USER_SETTINGS)
				?.Element(typeof(UserSettings).FullName)
				?.Elements(SETTING) ?? new XElement[0];
			...
			foreach (var element in settingElements)
			{
				var newSetting = new SettingStruct
				{
					...
					serializeAs = element.Attribute(SERIALIZE_AS) == null
						? "String"
						: element.Attribute(SERIALIZE_AS).Value,
        ...
        private void CreateEmptyConfig()
		{
			...
			var group = new XElement(typeof(UserSettings).FullName);
        ...
        private void SaveValuesToFile()
		{
			...
			//get the settings group (e.g. <Company.Project.Desktop.Configurator>)
			var settingsSection = import.Element(CONFIG).Element(USER_SETTINGS).Element(typeof(UserSettings).FullName);
